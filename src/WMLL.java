...
public class WMLL {
    ...
    private final String[] sleepstrings = { "A Gibbous moon is 3 quarters!",
	"1337 posts!", "iPeer <3 Boston", "/r/WMLL!", "Now on Reddit!",
	"What's My Arrow Level?",
	"Almost makes you wish for a nuclear winter!", "1 byte of posts!",
	"Kuurth for 1000!", "Paralympics!", "Olympics!", "London 2012!",
	"Would you kindly?", "Goodnight, PLAYERNAME!",
	"This is my bed. There are many like it, but this one is mine.",
	"If it fits, I sleeps!", "*fade to blackness*", "*water drip*",
	"Goodnight, Hero!", "ZzzzzZZz",
	"That'sssssssss a very nice bed you have there...",
	...
	this.debugClassPresent = (getClass().getClassLoader().getResource(
		"ipeer_wmll_debug") != null);
	...
	/*
	 * try { wmllCompatibility = new WMLLCompatibility(); } catch (Exception
	 * e) { shitBricks(1, e); } catch (Error e) { shitBricks(1, e); }
	 * compatDisabled =
	 * (getClass().getClassLoader().getResource("WMLLCompatibility.class")
	 * == null); if (compatDisabled)
	...
	/*
	 * if (getClass().getClassLoader().getResource("MotionTracker.class") !=
	 * null) { try { AlienRadar = true; alienRadar = new MotionTracker();
	 * alienError = ""; } catch (VerifyError e) { shitBricks(3, e);
	 * AlienRadar = false; } catch (Error e) { shitBricks(3, e); AlienRadar
	 * = false; } }
	 */
	if (getClass().getClassLoader().getResource(
		"com/thevoxelbox/voxelmap/VoxelMap.class") != null) {
	    try {
		ZansMinimap = true;
		zansMinimap = new VoxelMap(true); // Dear VoxelMap creators:
		// STOP CHANGING THIS CRAP.
		zanError = "";
	    } catch (VerifyError e) {
		shitBricks(4, e);
		ZansMinimap = false;
	 ...
	 * ); try { Properties a = new Properties(); a.load(new
	 * FileInputStream(outputOptionsFile)); options.putAll(a);
	 * saveOptions(); outputOptionsFile.deleteOnExit();
	 * debug("[WMLL] Succesfully merged files."); } catch (IOException w) {
	 * debug("[WMLL] Unable to merge files (IOException)\n");
	 * w.printStackTrace(); } }
	 */
	debug("[WMLL] WMLL is" + (debugClassPresent ? "" : " not")
	...
	/*
	 * try { debug("\t* "+wmllCompatibility.toString()); } catch (Exception
	 * e) { debug("\t* null"); }
	 */
	debug("\t* " + this.toString());
	try {
	...
	if (getWorld() != null && !wmllUpdateCheck.running && autoUpdateCheck) {
	    ...
	    System.out
		    .println("[WMLL] FATAL: World == NULL! -- Stopping update thread.");
	...
	if (getWorld() != null && !getWorldName().equals(lastWorld) && autoSeed) {
	    ...
	    boolean[] b = { Rei && ReiEnabled, ZansMinimap && ZansEnabled,
		AlienRadar && AlienEnabled };
	...
	if (!worldSeedSet && getWorld() != null && !isMultiplayer() && ranInit) {
		...
		if (options.getProperty("Seed:" + getWorldName().toLowerCase()) != null) {
		    worldSeedSet = true;
		    this.worldSeed = Long.valueOf(options.getProperty("Seed:"
			    + getWorldName().toLowerCase()));
		} else {
		    worldSeedSet = true;
		    this.worldSeed = ((js) MinecraftServer.F().a(0)).H();
		    /*
		     * Object obj = awq.b(); Field f =
		     * obj.getClass().getDeclaredField(getField("chatLines"));
		     * f.setAccessible(true); obj = f.get(obj);
		     *
		     * @SuppressWarnings("unchecked") List<String> a =
		     * (List<String>)obj;
		     *
		     * @SuppressWarnings("rawtypes") Iterator c = a.iterator();
		     * int e = 0; while (c.hasNext()) { e++; aut d =
		     * (aut)c.next(); String b = d.a(); //d.a(); if
		     * (b.startsWith("Seed: ")) { //aow.b().a(); //if
		     * (!isMultiplayer()) //a.remove(e - 1); c.remove(); long
		     * worldSeed = Long.parseLong(b.split("Seed: ")[1]);
		     * worldSeedSet = true; this.worldSeed = worldSeed;
		     * debug("[WMLL] Seed set to "+worldSeed); break; }
		     *
		     * }
		     */
		}
	...
	if (!ranInit) {
		...
		sendChatMessage("[WMLL] [ERR] WMLL was unable to render images, switching to tect-only.");
	    }
	...
	if (mcDebugOpen() || wmllF3Output) {
	    ...
	    /*
	     * if (satBar && !compatDisabled) { try {
	     * wmllCompatibility.drawSaturationBar(mc, this); } catch
	     * (NoSuchMethodError n) { satBar = false; } catch
	     * (NoClassDefFoundError n1) { satBar = false; } catch
	     * (NoSuchFieldError n2) { satBar = false; } catch
	     * (NullPointerException n3) { satBar = false; } // 12W40 ERROR
	     * catch (Error e) { satBar = false; } }
	     */
	    
	    /*
	     * if (RadarBro && !compatDisabled) try {
	     * wmllCompatibility.RadarBroRun(mc, this); } catch
	     * (NoSuchMethodError n) { } catch (NoClassDefFoundError n1) { }
		...
		String worldName = getWorldName() + " (" + isMultiplayer()
			+ ")";
		drawDebug(worldName, (getWindowSize().a() - (getFontRenderer()
			.a(worldName) + 1)), 0, 0xffffff);
		drawDebug(
			Integer.toString(getDimension()),
			(getWindowSize().a() - (getFontRenderer().a(
			Integer.toString(getDimension())) + 1)), 1,
			0xffffff);
		drawDebug(
			"Sleep: " + Boolean.toString(isPlayerSleeping()),
			(getWindowSize().a() - (getFontRenderer().a(
			"Sleep: "
			+ Boolean.toString(isPlayerSleeping())) + 1)),
			2, 0xffffff);
		drawDebug(
			Integer.toString(WMLLI),
			(getWindowSize().a() - (getFontRenderer().a(
			Integer.toString(WMLLI)) + 1)), 3, 0xffffff);
		int blockID = getBlockID(getPlayerCoordinates()[0],
			getPlayerCoordinates()[1] - 1,
			getPlayerCoordinates()[2]);
		drawDebug(
			Integer.toString(blockID),
			(getWindowSize().a() - (getFontRenderer().a(
			Integer.toString(blockID)) + 1)), 4, 0xffffff);
		try {
		    drawDebug(
			    "GUI: " + getGUI().toString(),
			    (getWindowSize().a() - (getFontRenderer().a(
			    "GUI: " + getGUI().toString()) + 1)), 5,
			    0xffffff);
		} catch (NullPointerException e) {
		    drawDebug(
			    "GUI: null",
			    (getWindowSize().a() - (getFontRenderer().a(
			    "GUI: null") + 1)), 5, 0xffffff);
		}
		drawDebug(
			Boolean.toString(canSlimesSpawnHere(x, z)),
			(getWindowSize().a() - (getFontRenderer().a(
			Boolean.toString(canSlimesSpawnHere(x, z))) + 1)),
			6, 0xffffff);
		
		long nextUpdate = ((lastUpdateCheck + 3600000) - System
			.currentTimeMillis()) / 1000;
		int hours = (int) (nextUpdate / 3600);
		int seconds = (int) nextUpdate % 60;
		int minutes = (int) (nextUpdate % 3600) / 60;
		String update = pad(hours) + ":" + pad(minutes) + ":"
			+ pad(seconds);
		String t = "Next update check: " + update;
		drawDebug(t,
			(getWindowSize().a() - (getFontRenderer().a(t) + 1)),
			7, 0xffffff);
		drawDebug(
			getPlayerController().toString(),
			(getWindowSize().a() - (getFontRenderer().a(
			getPlayerController().toString()) + 1)), 8,
			0xffffff);
		...
		drawDebug(a,
			...
			9, 0xffffff);
		drawDebug(
			Boolean.toString(isSeedSet()),
			(getWindowSize().a() - (getFontRenderer().a(
			Boolean.toString(isSeedSet())) + 1)), 10,
			0xffffff);
		a = "CP: " + getChunkProvider().toString();
		drawDebug(a,
			(getWindowSize().a() - (getFontRenderer().a(a) + 1)),
			12, 0xffffff);
		a = getGamePath().getAbsolutePath();
		drawDebug(a,
			(getWindowSize().a() - (getFontRenderer().a(a) + 1)),
			11, 0xffffff);
	    ...
	    WMLLCheckKeys();
	    // if (mc.s instanceof axl && !(mc.s instanceof WMLL_InGameMenu)/*
	    // && !(mc.s instanceof EllianGuiOptionScreen)*/ && useML)
	    if ((getGUIClassName().equals("axl") /* Vanilla */|| getGUIClassName()
		    .equals("net.minecraft.client.gui.GuiIngameMenu") /* Forge */)
		    && useML)
		mc.a(new WMLL_InGameMenu());
	    if (!Enabled || !shouldShow() || (WMLLI == 11 && classicOutput))
		return;
	    /*
	     * if (renderArmourDisplay && !isCreative()) { int gameHeight =
	     * getWindowSize().b(); int gameWidth = getWindowSize().a(); int
	     * armourX = (gameWidth / 2 - 93) + 0 * 16; for (int x =
	     * getPlayerArmour().length - 1; x >= 0; x--) { if
	     * (getPlayerArmour()[x] != null) {
	     * renderItemOnScreen(getPlayerArmour()[x], armourX, gameHeight -
	     * 65); armourX += 15; } }
	     *
	     * }
	     */
		...
		if (getCalendarDate().equals("66")) // My birthday
		...
		else if (getCalendarDate().equals("243")) // Roxy's birthday
		    // (<3)
		    lightString = "Happy birthday, Roxy!";
		else if (getCalendarDate().equals("202")) // WMLL's "birthday"
		    lightString = "Happy birthday, WMLL!";
		else if (getCalendarDate().equals(
			easters[(getYear() > 2014 ? getYear() - 2014 : 0)])) // Easter
		    // Sunday
		...
		else if (getCalendarDate().equals("31")) // Millie <3 RIP,
		    // honey.
		    lightString = "Millie <3";
		else if (getCalendarDate().equals("32")) { // Roxy and I's
		    // anniversary
		    int now = getYear();
		    int years = now - 2007;
		    ...
		    try {
			lightString = sleepstrings[(int) (System.nanoTime() % (long) sleepstrings.length)]
				.replaceAll("PLAYERNAME", getPlayerName());
		    } catch (Throwable e) {
			lightString = "Mr. Awesome Sleep Message is currently not available, please schedule an appointment.";
		    }
		...
		if (Arrays.asList(3, 4, 5, 8, 9, 10).contains(WMLLI)) {
		    ...
		    if ((isSeedSet())
			    || isMultiplayer()
			    || (isMultiplayer() && (getDimension() == -1 || getDimension() == 1)))
			out++;
		    double[] coordinates = getPlayerCoordinatesAsDouble();
		    NumberFormat d = new DecimalFormat("#0.00");
		    String coords = "(" + d.format(coordinates[0]) + ", "
			    + d.format(coordinates[1]) + ", "
			    + d.format(coordinates[2]) + ", "
			    + getPlayerDirection() + ")";
		    drawString(coords, 2, out, 0xffffff);
		    if (WMLLI != 5 && WMLLI != 10) {
			boolean showSeed = (!isMultiplayer() || isSeedSet()/*
			 * ||
			 * getWorldName
			 * (
			 * )
			 ...
			 */)
				&& showSeedWithCoords;
			if (showSeed)
			    drawString("Seed: " + getWorldSeed(), 2, out + 1,
				    0xffffff);
			// drawString("Facing: "+getPlayerDirection(playerPos[3]),
			// 2, out, 0xffffff);
			drawString("Biome: " + getBiome() + " (T: "
				+ getTemperature() + ", H: " + getHumidity()
				+ ")", 2, showSeed ? out + 2 : out + 1,
				0xffffff);
		    }
		}
		...
		if (Arrays.asList(1, 4, 6, 9).contains(WMLLI)) {
		    if (useImages) {
			wmllRenderer.renderIndicatorImages(
				light,
				getBlockID(playerPos[0], playerPos[1] - 1,
				playerPos[2]),
				getDimension(),
				canSlimesSpawnHere(playerPos[0], playerPos[2]),
				canBlockSeeTheSky(playerPos[0],
				playerPos[1] - 1, playerPos[2]));
			return;
		    }
		    boolean showSlimes = true;
		    if (isMultiplayer())
			showSlimes = isSeedSet();
		    String[] labels = { "Mobs", "Animals", "Trees", "Crops",
			"Mushrooms", "Slimes", "Ghasts", "Pigmen", "Blaze",
			"Endermen", "Grass" };
		    if (getDimension() == -1) { // Nether
			
			if (!isBlockInBlacklist(getBlockID(playerPos[0],
				playerPos[1] - 1, playerPos[2]))) {
			...
			if (light < 7
				&& !isBlockInBlacklist(getBlockID(playerPos[0],
				playerPos[1] - 1, playerPos[2])))
			...
			// Hostiles
			if ((light < 8 && !isBlockInBlacklist(getBlockID(
				playerPos[0], playerPos[1] - 1, playerPos[2])))
				&& (getBlockID(playerPos[0], playerPos[1] - 1,
				playerPos[2]) != 110 && !getBiome()
				.startsWith("MushroomIsland")))
			...
			if (playerIsStandingOnBlock(2))
				...
				drawString(
					(!canBlockSeeTheSky(playerPos[0],
					playerPos[1], playerPos[2]) ? "\247c"
					: "\247e")
					+ labels[1], 2, 2, 0xffffff);
			    else
			...
			if (showSlimes) {
				...
				boolean a = isBlockInBlacklist(getBlockID(
					playerPos[0], playerPos[1] - 1,
					playerPos[2]));
				if (light < 8 && !a)
				    drawString("\247a" + labels[5], 2, 3,
					    0xffffff);
				else if (light > 7 && !a)
				    drawString("\247e" + labels[5], 2, 3,
					    0xffffff);
				else
				    drawString("\247c" + labels[5], 2, 3,
					    0xffffff);
				
			    } else if (canSlimesSpawnHere(playerPos[0],
				    playerPos[2])
				    && !getBiome().startsWith("Swamp"))
				if ((playerPos[1] - 1) <= 40)
				    drawString("\247a" + labels[5], 2, 3,
					    0xffffff);
				else
		    ...
		    // Crops
		    if (playerIsStandingOnBlock(60)
			    && (getLightLevel(playerPos[0], playerPos[1] + 1,
			    playerPos[2]) > 8 || canBlockSeeTheSky(
			    playerPos[0], playerPos[1], playerPos[2])))
			drawString("\247a" + labels[3],
				getDimension() == 1 ? 55 : 40, 1, 0xffffff);
		    else
			drawString("\247c" + labels[3],
				getDimension() == 1 ? 55 : 40, 1, 0xffffff);
		    
		    // Trees
		    if ((playerIsStandingOnBlock(2) || playerIsStandingOnBlock(3))
			    && (light > 8 || canBlockSeeTheSky(playerPos[0],
			    playerPos[1], playerPos[2])))
			drawString("\247a" + labels[2], getDimension() == 1 ? 2
				: 40, 2, 0xffffff);
		    else
			drawString("\247c" + labels[2], getDimension() == 1 ? 2
				: 40, 2, 0xffffff);
		    
		    // Mushrooms
		    if ((playerIsStandingOnBlock(110) || light < 13)
			    && !isBlockInBlacklist(getBlockID(playerPos[0],
			    playerPos[1] - 1, playerPos[2])))
			drawString("\247a" + labels[4],
				getDimension() == 0 ? 40 : 40,
				getDimension() == 1 ? 2 : 3, 0xffffff);
		    else
			drawString("\247c" + labels[4],
				getDimension() == 0 ? 40 : 40,
				getDimension() == 1 ? 2 : 3, 0xffffff);
		    
		    // Grass
		    if ((playerIsStandingOnBlock(3) && light > 8))
			drawString("\247a" + labels[10],
				getDimension() == -1 ? 40 : 2,
				getDimension() == 1 ? 3
				: getDimension() == -1 ? 4
				: !showSlimes ? 3 : 4, 0xffffff);
		    else
			drawString("\247c" + labels[10],
				getDimension() == -1 ? 40 : 2,
				getDimension() == 1 ? 3
				: getDimension() == -1 ? 4
				: !showSlimes ? 3 : 4, 0xffffff);
	...
	return generateLightString(options.getProperty("lightString",
		"Light level: %LightLevel%"));
    }
    
    public String generateLightString(String s) { // [Roxy] Now Case insensitive
	// TODO: Don't actually need to do anything here, this is just so I can
	// find this part easily.
	int x = getPlayerCoordinates()[0];
	...
	Pattern SkyLight = Pattern.compile("%skylight%",
		Pattern.CASE_INSENSITIVE);
	Pattern BlockLight = Pattern.compile("%blocklight%",
		Pattern.CASE_INSENSITIVE);
	Pattern RawLight = Pattern.compile("%rawlight%",
		Pattern.CASE_INSENSITIVE);
	Pattern Light = Pattern.compile("%LightLevel%",
		Pattern.CASE_INSENSITIVE);
	Pattern sunLight = Pattern.compile("%sunlight%",
		Pattern.CASE_INSENSITIVE);
	...
	Pattern fullCompass = Pattern.compile("%fullcompass%",
		Pattern.CASE_INSENSITIVE);
	Pattern chunkUpdates = Pattern.compile("%(cu|chunkupdates)%",
		Pattern.CASE_INSENSITIVE);
	Pattern heading = Pattern
		.compile("%heading%", Pattern.CASE_INSENSITIVE);
	...
	Pattern clock2 = Pattern
		.compile("%12hclock%", Pattern.CASE_INSENSITIVE);
	...
	Pattern localTime = Pattern.compile("%localtime%",
		Pattern.CASE_INSENSITIVE);
	Pattern localTime12h = Pattern.compile("%12hlocaltime%",
		Pattern.CASE_INSENSITIVE);
	Pattern entities = Pattern.compile("%entities%",
		Pattern.CASE_INSENSITIVE);
	Pattern entities2 = Pattern.compile("%fullentities%",
		Pattern.CASE_INSENSITIVE);
	Pattern moonPhase = Pattern.compile("%moon(phase)?%",
		Pattern.CASE_INSENSITIVE);
	String lightLevel = (a < 8 && colourLowLight ? "\247c" : "")
		+ Integer.toString(a) + "\247r";
	a = getSavedBlockLight(x, y, z);
	String blockLight = (a < 8 && colourLowLight ? "\247c" : "")
		+ Integer.toString(a) + "\247r";
	a = getRawLightLevel(x, y, z);
	String rawLight = (a < 8 && colourLowLight ? "\247c" : "")
		+ Integer.toString(a) + "\247r";
	a = getSunLight(x, y, z);
	String skyLight = (a < 8 && colourLowLight ? "\247c" : "")
		+ Integer.toString(a) + "\247r";
	...
	String coords = "(" + n.format(x1) + ", " + n.format(y1) + ", "
		+ n.format(z1) + ", " + getPlayerDirection((int) f1) + ")";
	...
	Pattern indicators = Pattern.compile(
		"%ind:([\\p{Alnum}\\p{Punct}&&[^\\\\ ]]+)%",
		Pattern.CASE_INSENSITIVE);
	...
	s = m.replaceAll(moonPhases[getMoonPhase()]
		+ (WMLLDebugActive() ? ", " + getMoonPhase() : ""));
	Pattern debug = Pattern.compile(
		"%debug:([\\p{Alnum}\\p{Punct}&&[^\\\\ ]]+)%",
		Pattern.CASE_INSENSITIVE);
	m = debug.matcher(s);
	while (m.find()) {
	...
	Pattern count = Pattern.compile(
		"%count:([\\p{Alnum}\\p{Punct}&&[^\\\\ ]]+)%",
		Pattern.CASE_INSENSITIVE);
	m = count.matcher(s);
	while (m.find()) {
	    String bdata = m.group().replaceAll("%count:|%", "").toLowerCase();
	    s = s.replaceAll("%count:" + bdata + "%", formatCount(bdata)
		    + "\247r");
	}
	
	Pattern formats = Pattern.compile("&([0123456789abcdefklmnor])",
		Pattern.CASE_INSENSITIVE);
	m = formats.matcher(s);
	while (m.find()) {
	...
	Pattern dimension = Pattern.compile("%dimension%",
		Pattern.CASE_INSENSITIVE);
	m = dimension.matcher(s);
	s = m.replaceAll(Integer.toString(getDimension()));
	
	Pattern dimensionName = Pattern.compile("%dimensionname%",
		Pattern.CASE_INSENSITIVE);
	...
	ye[] inventory = getPlayerInventory().a;
	if (name.startsWith("slot")) {
	    int slot = Integer.valueOf(name.replaceAll("slot", ""));
	    ye item = inventory[slot];
	    String itemName = "";
	...
	for (ye item : inventory) {
	    if (item != null && (id > -1 && getItemId(item) == id)
		    || getInternalNameForItem(item).equals(name)
		    || getItemName(item, true).contains(name)) {
		return getItemName(item) + ": "
			+ getItemQuantity(getInternalItemName(item));
	    }
	}
	return "";
    }
    ...
    public ye getItemInSlot(int slot) {
	try {
    ...
    public String getItemName(ye item) {
	return getItemName(item, false);
    }
    
    public String getItemName(ye item, boolean lower) {
	try {
	    if (lower)
    ...
    public int getItemId(ye item) {
	return item.d;
    }
    
    public String getInternalItemName(ye item) {
	return getInternalNameForItem(item);
    }
    
    public String getInternalNameForItem(ye item) {
	try {
	    return item.a();
	} catch (NullPointerException e) {
	    return "";
    ...
    public ye[] getPlayerArmour() {
	return entityPlayer().bn.a;
    }
    
    public boolean itemHasEnchant(int enchantID, ye wg) {
	return aaw.a(51, wg) > 0;
    }
	...
	if (v.equals("arrows")) {
		...
		if (!isSlotEmpty(x1)
			&& getInternalItemNameForSlot(x1).equals("item.arrow"))
		    arrows += items[x1].b;
	    }
	    return arr + arrows;
	} else if (v.equals("armour")) {
	    ye[] armour = getPlayerArmour();
	    String o = "";
		...
		if (debugClassPresent && debugActive)
		    return "N: " + itemName + ", MaxDur: " + itemMaxDur
			    + ", CurrDur: " + itemCurrDur + ", Data: "
			    + itemData + ", ID: " + itemID + ", HasData: "
			    + hasData;
		if (itemMaxDur > 0) {
		    return itemName
			    + ": "
			    + new DecimalFormat("##")
			    .format(((double) itemCurrDur / itemMaxDur) * 100.00)
			    + "% durability, " + itemCurrDur + " uses.";
		}
	...
	if (mode == 1) {
	    ...
	    return a + ":" + time.split(":")[1] + ":" + time.split(":")[2]
		    + " " + (PM ? "PM" : "AM");
	}
	...
	if (!isMultiplayer())
	    return sspServer().M();
	try {
	    SocketAddress serverIP = entityPlayer().a.g().c();
    ...
    public awf getWindowSize() {
	return new awf(mc.u, mc.d, mc.e);
    }
    ...
    private aul getGameSettings() {
	return mc.u;
    }
    
    public boolean isMultiplayer() {
	/*
	 * if (useML) return !ModLoader.getMinecraftInstance().B();
	 */// Disabled due to 1.6 changes.
	return !mc.B();
    }
	...
	if (y < 0 || y > 255)
	    return 0;
	return getChunk(x, z).a(ach.b, x & 0xf, y, z & 0xf);
    }
	...
	if (y < 0 || y > 255)
	    return 0;
	return getChunk(x, z).a(ach.a, x & 0xf, y, z & 0xf);
    }
    
    public int getBlockLight(int i, int j, int k) {
	if (j < 0 || j > 255)
	    return 0;
	return getChunk(i, k).a(ach.b, i & 0xf, j, k & 0xf);
    }
	...
	return NumberFormat.getPercentInstance().format(
		getBiomeGenBase().a(getPlayerCoordinates()[0],
		getPlayerCoordinates()[2]).F);
    }
    
    private String getHumidity() {
	return NumberFormat.getPercentInstance().format(
		getBiomeGenBase().a(getPlayerCoordinates()[0],
		getPlayerCoordinates()[2]).G);
    }
    
    private acv getBiomeGenBase() {
	return getWorld().u();
    }
    ...
    public bdi entityPlayer() {
	return mc.h;
    }
    
    public bdi thePlayer() {
	return mc.h;
    }
    
    public beh playerEntity() {
	return mc.k;
    }
    ...
    public bdc getPlayerController() {
	return mc.c;
    }
    ...
    public als worldInfo() {
	return getWorld().x;
    }
    
    // protected Minecraft getMCInstance() {
    protected atv getMCInstance() {
	return mc;
    }
    
    private adr getChunk(int x, int z) {
	...
	return (dimensionNames.containsKey(getDimension()) ? dimensionNames
		.get(getDimension()) : getWorld().t.l());
    }
    
    private boolean canSlimesSpawnHere(int x, int z) {
	if (isSeedSet()) {
	    adr chunk = getChunk(x, z);
	    int g = chunk.g;
	    int h = chunk.h;
	    return new Random(getWorldSeed() + (long) (g * g * 0x4c1906)
		    + (long) (g * 0x5ac0db) + (long) (h * h) * 0x4307a7L
		    + (long) (h * 0x5f24f) ^ 0x3ad8025fL).nextInt(10) == 0;
	}
    ...
    private aei getWorldProvider() {
	return getWorld().t;
    }
    
    private acg getChunkProvider() {
	...
	int[] a = { ls.c(thePlayer().u), ls.c(thePlayer().v - 1),
	    ls.c(thePlayer().w),
	    ls.c((double) ((thePlayer().A * 4F) / 360F) + 0.5D) & 3,
	    (int) thePlayer().u, (int) thePlayer().v, (int) thePlayer().w };
	return a;
    }
    
    public double[] getPlayerCoordinatesAsDouble() {
	double[] a = { thePlayer().u, thePlayer().v, thePlayer().w,
	    thePlayer().E.b,
	    ls.c((double) ((thePlayer().A * 4F) / 360F) + 0.5D) & 3 };
	return a;
    }
	...
	if (worldSeedSet)
	    ...
	    try {
		return Long.parseLong(options.getProperty("Seed:"
			+ getWorldName().toLowerCase(), "0"));
	...
	for (String a : lines) {
		...
		getFontRenderer().a(a, w - (getFontRenderer().a(a1) + (i - 1)),
			textpos + (j * 10 + (10 * l++)), k);
	    else if (outputLocation == 2) // Bottom left
		getFontRenderer().a(a, i,
			h - (textpos + (j * 10 + (10 * l++)) + 8), k);
	...
	try {
	    ...
	    options.setProperty("OutputLocation",
		    Integer.toString(outputLocation));
	    options.setProperty("OverrideIngameF3",
		    Boolean.toString(wmllOverrideF3));
	    options.setProperty("showSeedWithCoords",
		    Boolean.toString(showSeedWithCoords));
	    options.setProperty("F3Type", Integer.toString(F3Type));
	    options.setProperty("classicOutput",
		    Boolean.toString(classicOutput));
	    options.setProperty("autoUpdateCheck",
		    Boolean.toString(autoUpdateCheck));
	    ...
	    options.setProperty("showUnderGUIs",
		    Boolean.toString(showUnderGUIs));
	    options.setProperty("showWorldName",
		    Boolean.toString(showWorldName));
	    options.setProperty("renderArmourDisplay",
		    Boolean.toString(renderArmourDisplay));
	    options.setProperty("colourLowLight",
		    Boolean.toString(colourLowLight));
	    options.setProperty("forceAutohideObey",
		    Boolean.toString(forceAutohideObey));
	    options.store(new FileOutputStream(settingsFile),
		    "WMLL Config File - Do not edit unless you know what you're doing!");
	    // if (!outputOptions.isEmpty())
	    // outputOptions.store(new FileOutputStream(outputOptionsFile),
	    // "WMLL's Output Options File - only edit if you know waht you're doing!");
	    debug("[WMLL] Options saved.");
	    // debug(options.toString()+"\n"+outputOptions.toString());
	} catch (Exception e) {
	    debug("[WMLL] Unable to save options: " + e.getMessage());
	    e.printStackTrace();
	}
    }
	...
	try {
		...
		File a = new File(getGamePath(), "WMLL.properties");
		if (a.exists()) {
	    ...
	    firstRun = Integer.parseInt(options.getProperty("version", "0")) < propertiesVersion ? true
		    : false;
	    // if (!debugClassPresent)
	    // debugActive = Boolean.parseBoolean(options.getProperty("WMLLD",
	    // "false"));
	    WMLLI = Integer.parseInt(options.getProperty("WMLLI", "0"));
	    useImages = Boolean.parseBoolean(options.getProperty("useImages",
		    "false"));
	    TextColour = Integer.parseInt(options.getProperty("TextColour",
		    "-1"));
	    F4Key = Integer.parseInt(options.getProperty("F4", "62"));
	    clockSetting = Integer.parseInt(options.getProperty("clockSetting",
		    "2"));
	    outputLocation = Integer.parseInt(options.getProperty(
		    "OutputLocation", "0"));
	    wmllOverrideF3 = Boolean.parseBoolean(options.getProperty(
		    "OverrideIngameF3", "false")); // Now defaults to false due
	    // to Minecraft's native
	    // Shift + F3
	    F3Type = Integer.parseInt(options.getProperty("F3Type", "0"));
	    showSeedWithCoords = Boolean.parseBoolean(options.getProperty(
		    "showSeedWithCoords", "true"));
	    classicOutput = Boolean.parseBoolean(options.getProperty(
		    "classicOutput", "false"));
	    autoUpdateCheck = Boolean.parseBoolean(options.getProperty(
		    "autoUpdateCheck", "true"));
	    satBar = Boolean.parseBoolean(options.getProperty("SaturationBar",
		    "false"));
	    ReiEnabled = Boolean.valueOf(options.getProperty("Compat-Rei",
		    "true"));
	    AlienEnabled = Boolean.valueOf(options.getProperty("Compat-Alien",
		    "true"));
	    ZansEnabled = Boolean.valueOf(options.getProperty("Compat-Zans",
		    "true"));
	    forgeEnabled = Boolean.valueOf(options.getProperty("Compat-Forge",
		    "true"));
	    showUnderGUIs = Boolean.valueOf(options.getProperty(
		    "showUnderGUIs", "true"));
	    showWorldName = Boolean.valueOf(options.getProperty(
		    "showWorldName", "true"));
	    renderArmourDisplay = Boolean.valueOf(options.getProperty(
		    "renderArmourDisplay", "false"));
	    colourLowLight = Boolean.valueOf(options.getProperty(
		    "colourLowLight", "true"));
	    forceAutohideObey = Boolean.valueOf(options.getProperty(
		    "forceAutohideObey", "false"));
	    debug("[WMLL] Loaded options.");
	    // debug(options.toString()+"\n"+outputOptions.toString());
	    if (firstRun || updatedFormat)
		saveOptions();
	} catch (Exception e) {
	    debug("[WMLL] Unable to load options: " + e.getMessage());
	}
    }
	...
	if (Keyboard.isKeyDown(F4Key)
		&& System.currentTimeMillis() - lastF4Press > 250) {
	    lastF4Press = System.currentTimeMillis();
	    if (Keyboard.isKeyDown(29) && getGUI() == null)
		mc.a(new WMLLOptionsMenu(this));
	    else if (getGUI() == null) {
		// && !(mc.s instanceof acr/*GuiChat*/) && !(mc.s instanceof
		// ars/*Sign Editing*/) && !(mc.s instanceof hw/*Book
		// Editing*/)) {
		if (WMLLDebugActive()) {
		    showDebug = !showDebug;
		}
	...
	sendChatMessage("WMLL \247c" + newver
		+ "\247f is now available for Minecraft \247c" + mcVersion);
    }
	...
	try {
	    ...
	    if (i < 0)
		WMLLI = i = 11;
	    return Boolean.parseBoolean(options.getProperty("Output" + i,
		    "true"));
	} catch (Exception e) {
	    e.printStackTrace();
	    return true;
	...
	int x = 0;
	for (; Boolean.parseBoolean(options.getProperty("Output" + x, "true")) == false
		&& x <= 11; x++) {
	}
	return x > 11;
    }
	...
	switch (id) {
	    ...
	    case 2:
		System.err
			.println("[WMLL] Error creating Rei's Minimap Compatibility.");
		printStackTrace(e);
		Rei = ReiUseMl = false;
		reiError = e.toString();
		break;
	    case 3:
		System.err
			.println("[WMLL] Error creating Alien Radar Compatibility.");
		AlienRadar = false;
		printStackTrace(e);
	    ...
	    case 4:
		System.err
			.println("[WMLL] Error creating Zan's Minimap Compatibility.");
		ZansMinimap = false;
		printStackTrace(e);
		zanError = e.toString();
		break;
    ...
    public void printStackTrace(avv aum, Throwable e) {
	System.err
	...
	System.err.println("\nCP: " + System.getProperty("java.class.path")
		+ "\n");
	if (getGUI() != null)
	    System.err.println("Affected GUI: "
		    + getGUI().toString().split("@")[0] + "\n");
	System.err.println("Error type: " + e.toString() + "\nInitial Method: "
		+ e.getStackTrace()[0].toString().split("\\(")[0] + "\n");
	e.printStackTrace();
	System.err.println("\n ---- End of Debug Info ---- \n");
    }
	...
	return /* !a && */Boolean.parseBoolean(options.getProperty("World-"
		+ getWorldName(), "true"))
		&& !Boolean.parseBoolean(options.getProperty("AllOutputsOff",
		"false"));
    }
	...
	a.nextInt();
	return a.nextInt(3) != 0 ? false
		: (x != (cx << 4) + 4 + a.nextInt(8) ? false : z == (cz << 4)
		+ 4 + a.nextInt(8));
    }
